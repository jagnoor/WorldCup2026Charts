...
### The tournament's over — the archive lives on. Every match, every round, the whole story.
...
**The 2026 World Cup is over** — Spain lifted the trophy at MetLife on July 19. The site now runs
in **retrospective mode**: the same hub, re-branded as a permanent results archive (final scores
frozen locally, no more polling), plus a printable keepsake and a long-read on the tournament's
biggest controversy ([The Whistle Ledger](https://jagnoor.github.io/WorldCup2026Charts/argentina.html) —
Argentina & the officiating, tested against primary data).

During the tournament it was **two live tools**, and you never need to touch code to use either one:
...
```
  ...
  3. ✎  OVERRIDES        overrides.json — the owner can hand-enter a result the feed
         (top layer)      hasn't logged yet. Visibly tagged ✎ manual. (Rarely needed
              ...
              │
              ▼
  4. 🏁  ARCHIVE          results.json — the frozen final archive (all 104 results,
         (last resort)    source:"archive"). Shown if every feed is unreachable — and
                          since the tournament ended, it IS the permanent record.
```
