...
(function () {
  ...
  let RETRO = false;                // true once every match is decided → retrospective/archive mode
  ...
  const TX = {
    ...
    retroH1: ES ? 'Retrospectiva <span class="grad-text">2026</span>' : 'The 2026 <span class="grad-text">Retrospective</span>',
    retroSub: ES
      ? 'El torneo terminó. Cada partido, cada ronda, cada marcador — el archivo completo de cómo se llegó a la final.'
      : 'The tournament is over. Every match, every round, every scoreline — the full archive of how it all unfolded.',
    bArchive: ES ? '🏁 RESULTADOS FINALES · ARCHIVO' : '🏁 FINAL RESULTS · ARCHIVE',
    archiveCaveat: ES
      ? 'Este es el <b>archivo final</b> del torneo — una instantánea de los resultados completos tomada el 20 de julio de 2026 de los datos públicos de ESPN. El torneo terminó; estos resultados son definitivos. No oficial: para registros oficiales, consulta a la FIFA.'
      : 'This is the tournament’s <b>final archive</b> — a snapshot of the completed results taken July 20, 2026 from ESPN’s public data. The tournament is over; these results are final. Unofficial: for official records, consult FIFA.',
    retroReads: ES ? 'Lecturas de la retrospectiva' : 'Retrospective reads',
    whistleTitle: ES ? 'El Libro del Silbato — Argentina y el arbitraje' : 'The Whistle Ledger — Argentina & the officiating',
    whistleDesc: ES
      ? 'Los ocho partidos de Argentina, cada decisión disputada — ponderada, contrastada y con fuentes. Un análisis honesto de la polémica arbitral del torneo.'
      : 'All eight Argentina matches, every contested call — weighted, tested and sourced. An honest look at the tournament’s biggest officiating controversy.',
    keepsakeTitle: ES ? 'El póster de recuerdo' : 'The keepsake wall chart',
    keepsakeDesc: ES
  ...
  function allDecided() { return STAGES.every(s => S.M.filter(m => s.codes.includes(m[5])).every(m => isDecided(m[0]))); }
  function defaultTab() {
    if (allDecided()) return 'overview';                     // retrospective → the full-archive view
  ...
  function funnel() {
    ...
    const curIdx = RETRO ? order.indexOf('CH') : order.indexOf(cur.key === 'FIN' ? 'FIN' : cur.key);
    return '<div class="kfunnel">' + steps.map((s, i) => {
      const cls = i < curIdx ? 'past' : i === curIdx ? 'now' : 'future';
      const liveCount = (i === curIdx && cur.key !== 'GS') ? (RETRO ? 1 : sv.remaining.length) : s.n;
  ...
  function dataStrip(v) {
    ...
    const badge = isArchive ? `<span class="kbadge archive">${TX.bArchive}</span>`
      : isSample ? `<span class="kbadge sample">${TX.bSample}</span>`
  ...
  function dataCaveat() {
    ...
    const lines = [];
    if (SOURCE === 'archive') lines.push(TX.archiveCaveat);
    else if (isEspn && RETRO) lines.push(TX.archiveCaveat);
    else if (isEspn) lines.push(ES
  ...
  function render() {
    ...
    RETRO = allDecided();                                    // tournament over → archive mode
    ...
    const h1 = RETRO ? TX.retroH1 : `${esc(TX.road)} <span class="grad-text">Final</span>`;
    const sub = RETRO ? `<p class="kretro-sub">${esc(TX.retroSub)}</p>` : '';
    $('#kapp').innerHTML =
      `<header class="khero">
         <span class="status-chip"><span class="dot"></span><span>${esc(headerStatus())}</span></span>
         <h1 class="khead">${h1}</h1>
         ${sub}
         ${dataStrip(v)}
         ${tzControl()}
         ${funnel()}
  ...
  function applyData(data) {
    ...
    FEED_KO = (data && data.koTeams) || {}; FEED_IDS = (data && data.ids) || {};
    UPDATED = (data && Date.parse(data.updated)) || 0; PULLED = Date.now();
    SOURCE = (data && data.source) || '';
    NOTE = (data && data.note) || ''; SRCURL = ''; NOW = UPDATED || Date.now();
  }
  function loadSample() {
  ...
  loadData(false);             // then pull the feed (falls back to the bundled archive)
  // While the tournament ran this polled every minute; in retrospective mode the
  // results are final, so background refreshing is pointless — skip it.
  setInterval(() => { if (!RETRO) loadData(false); }, 60000);
  document.addEventListener('visibilitychange', () => { if (!document.hidden && !RETRO) loadData(false); });
})();
